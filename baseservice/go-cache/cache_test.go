...
func TestAdd(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	ok := tc.Add("foo", "bar", DefaultExpiration)
	if !ok {
		t.Error("Couldn't add foo even though it shouldn't exist")
	}
	ok = tc.Add("foo", "baz", DefaultExpiration)
	if ok {
...
func TestGetWithExpiration(t *testing.T) {
	...
	if !expiration.IsZero() {
		t.Error("expiration for a is not a zeroed time", expiration)
	}
	...
	e, _ := tc.items.Load("e")
	if expiration.UnixNano() != e.(*Item).Expiration {
...
set use => 34.62988ms , len => 1431246
read use => 7.767448ms , len => 2845915

set use => 6.352653ms , len => 1450176
read use => 1.114749ms , len => 2785297

set use => 16.403218ms , len => 1444797
read use => 12.836103ms , len => 2685274

set use => 23.388394ms , len => 1486536
read use => 4.782721ms , len => 2833385

set use => 18.015716ms , len => 1393993
read use => 18.308408ms , len => 2642611
*/

/*
go-cache

set use => 43.442498ms , len => 1477870
read use => 10.420612ms , len => 2705665

set use => 28.217642ms , len => 1466777
read use => 1.657943ms , len => 2786748

set use => 19.951862ms , len => 1489377
read use => 5.891418ms , len => 2849206

set use => 29.405818ms , len => 1484888
read use => 2.749085ms , len => 2865779

set use => 21.779391ms , len => 1491974
read use => 1.153393ms , len => 2705879
*/

/*
func init() {
	p, _ = ants.NewPool(310000)
}

var p *ants.Pool

func TestHighConcurrency(t *testing.T) {
	highConcurrencySyncSetNX()
	fmt.Println()
	fmt.Println()
	fmt.Println()
	highConcurrencySyncSet()
}

func highConcurrencySyncSetNX() {
	mem := New(10*time.Second, 10*time.Second)
	fmt.Println("======使用sync.Map======")

	setT := make([]time.Duration, 0, 100000)
	readT := make([]time.Duration, 0, 1000000)
	var state int32
	for i := 0; i < 200000; i++ {
		p.Submit(func() {
			func(index int) {
				for {
					now := time.Now()
					mem.Get(fmt.Sprintf("item%d", index))
					readT = append(readT, time.Now().Sub(now))
	...
	go func() {
		for {
			printMemStats(time.Now().Format("2006-01-02 15:04:05"))
			time.Sleep(time.Second)
			if atomic.LoadInt32(&state) == 1 {
				return
			}
	...
	fmt.Println("set use =>", useTime(setT), ", len =>", len(setT))
	fmt.Println("read use =>", useTime(readT), ", len =>", len(readT))
}

func highConcurrencySyncSet() {
	mem := New(10*time.Second, 10*time.Second)
	fmt.Println("======使用sync.Map======")
	...
	for i := 0; i < 100000; i++ {
		p.Submit(func() {
			func(index int) {
				for {
					now := time.Now()
					mem.Set(fmt.Sprintf("item%d", index), fmt.Sprintf("value%d", index), 5*time.Second)
					setT = append(setT, time.Now().Sub(now))
					if atomic.LoadInt32(&state) == 1 {
						return
					}
					time.Sleep(time.Second)
...
func highConcurrencyGoCache() {
	mem := gocache.New(10*time.Second, 10*time.Second)
	fmt.Println("======使用原版go-cache======")
...
func useTime(num []time.Duration) time.Duration {
	var sum time.Duration
	for _, v := range num {
		sum += v
	}
	if len(num) > 0 {
		return sum / time.Duration(len(num))
	}
	return 0
}
